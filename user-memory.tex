...
\begin{frame}[fragile=singleslide]{Les pools de mémoires}
  ...
  \item Principe du salb dans le noyau (\file{/proc/slabinfo})
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile=singleslide]{Résumé agencement mémoire}
    \pgfimage[width=10cm]{pics/linuxFlexibleAddressSpaceLayout}
\end{frame}

\begin{frame}[fragile=singleslide]{Fonctionnement Linux}
    \pgfimage[width=10cm]{pics/memoryDescriptorAndMemoryAreas}
\end{frame}
...
\begin{frame}[fragile=singleslide]{dmalloc}
  ...
  \item \cmd{gcc -g debug_mem.c -ldmalloc -o debug_mem_dmalloc}
  \item \cmd{dmalloc -DV}
  \item \cmd{dmalloc -tV}
  \item \cmd{dmalloc high -m error-abort -l dmalloc.out}
  \item
    \cmd{echo 123456 | DMALLOC_OPTIONS=debug=0xcb4ed2b,log=dmalloc.out ./debug_mem_dmalloc}
  \item \cmd{less dmalloc.out}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile=singleslide]{Libefence, DUMA et kmemcheck}
  \begin{itemize}
  \item \cmd{gcc -g debug_mem.c -o debug_mem_duma}
  \item \cmd{ulimit -c unlimited}
  \item \cmd{rm core}
  \item
    \cmd{echo 1234 | LD_PRELOAD=/usr/lib/libduma.so.0.0.0 DUMA_FILL=1 DUMA_PROTECT_FREE=1 ./debug_mem_duma}
  \item \cmd{gdb -core core debug_mem_duma}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile=singleslide]{Valgrind}
  \begin{itemize} 
  \item \cmd{gcc -g debug_mem.c -o debug_mem_valgrind}
  \item \cmd{echo 123456 | valgrind --leak-check=yes ./debug_mem_valgrind}
  \end{itemize}  
\end{frame}

\begin{frame}[fragile=singleslide]{Mudflap}
  \begin{itemize}
  \item \cmd{sudo apt-get install libmudflap0-4.6-dev}
  \item \cmd{gcc -g -fmudflap debug_mem.c -lmudflap  -o debug_mem_mudflap}
  \item \cmd{echo 12345 | MUDFLAP_OPTIONS='-help' ./debug_mem_mudflap}
  \item \cmd{echo 12345 | MUDFLAP_OPTIONS='-print-leaks -check-initialization' ./debug_mem_mudflap}
  \end{itemize}
\end{frame}
